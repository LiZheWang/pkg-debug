...
module.exports = function () {
  ...
  /*---------------参数配置  start------------------*/
  const BASE_DIR = config.baseDir || './api';
  const BASE_DTO_DIR = `${BASE_DIR}/dto/index.d.ts`;
  const BASE_FN_DIR = `${BASE_DIR}/index.ts`;
  const TsNamespace = config.tsNamespace || 'NEOAPI';
  ...
  function typeRelevanceDto(schema) {
    try {
      ...
      return type.startsWith(TsNamespace);
    } catch (e) {
      return false;
    }
  }
  ...
  function createDTO(definitions) {
    try {
      ...
      txt += `declare namespace ${TsNamespace} { ${BR}`;
      ...
      txt += `}`;
      fs.outputFileSync(BASE_DTO_DIR, txt);
    } catch (e) {
  ...
  function createAPi(paths) {
    try {
      ...
      fs.outputFileSync(BASE_FN_DIR, txt);
    } catch (e) {
