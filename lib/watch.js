  ...
  if (sources && sources.length) {
    ...
    for (const source of sources) {
      ...
      if (!fs.existsSync(distPackageDir)) {
        console.log(chalk.red(`${source.distDirName}项目node_modules下${package.name}目录不存在,path：${distPackageDir}`))
        continue;
      }
    ...
    console.log(chalk.red('debug包提示：根目录必须要有pkg-debug-config.js配置文件，并且配置sources字段'))
    console.log(chalk.yellow('更多配置，请查看文档：http://www.baidu.com'))
  }
}
